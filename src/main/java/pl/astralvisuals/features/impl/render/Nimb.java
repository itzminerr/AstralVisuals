...
public final class Nimb {
    ...
    public void onWorldRender(WorldRenderEvent e) {
        ...
        for (class_1657 pl : p.mc.field_1687.method_18456()) {
            ...
            double px = class_3532.method_16436(pt, pl.field_6014, pl.method_23317());
            double py = class_3532.method_16436(pt, pl.field_6036, pl.method_23318());
            double pz = class_3532.method_16436(pt, pl.field_5969, pl.method_23321());

            double hy = py + pl.method_17682() + 0.12 + p.haloYOffset.getValue();

            double dx = camPos.field_1352 - px;
            double dy = camPos.field_1351 - hy;
            double dz = camPos.field_1350 - pz;

            if ((dx * dx + dy * dy + dz * dz) > maxSq) continue;

            float bodyYawDeg = class_3532.method_17821(pt, pl.field_6220, pl.field_6283);
    ...
    private void renderHalo(class_4587 ms, class_4184 cam, class_1657 player,
                            double px, double py, double pz, double bodyYawRad) {

        double hy = py + player.method_17682() + 0.12 + p.haloYOffset.getValue();

        float t = (System.currentTimeMillis() % 1_000_000L) / 1000.0f;
        float bobAmp = class_3532.method_15363(p.haloThickness.getValue() * 0.8f, 0.0f, 0.12f);
        ...
        class_243 viewDir = cam.method_19326().method_1023(px, hy, pz);
        double vlen = viewDir.method_1033();
        if (vlen > 1.0E-6) viewDir = viewDir.method_1021(1.0 / vlen);
        else viewDir = new class_243(0, 0, 1);

        class_243 lightDir = new class_243(0.35, 1.0, 0.25);
        double llen = lightDir.method_1033();
        if (llen > 1.0E-6) lightDir = lightDir.method_1021(1.0 / llen);
        else lightDir = new class_243(0, 1, 0);
        ...
        ms.method_22903();
        ms.method_22904(px, hy, pz);
        qTmp.identity().rotateY((float) bodyYawRad);
        ms.method_22907(qTmp);
    ...
    private void drawHaloTorusFast(class_4587 ms, float majorR, float tubeR, int argb, class_243 viewDir, class_243 lightDir) {
        int a0 = (argb >>> 24) & 0xFF;
        int r0 = (argb >>> 16) & 0xFF;
        int g0 = (argb >>> 8) & 0xFF;
        int b0 = argb & 0xFF;
        ...
        for (int i = 0; i < majorSeg; i++) {
            ...
            for (int j = 0; j < minorSeg; j++) {
                ...
                bb.method_22918(m, xA, yA, zA).method_39415(cA);
                bb.method_22918(m, xB, yB, zB).method_39415(cB);
                bb.method_22918(m, xC, yC, zC).method_39415(cC);

                bb.method_22918(m, xA, yA, zA).method_39415(cA);
                bb.method_22918(m, xC, yC, zC).method_39415(cC);
                bb.method_22918(m, xD, yD, zD).method_39415(cD);
            }
        }
    ...
    private int shadeHalo(float nx, float ny, float nz, class_243 viewDir, class_243 lightDir, int r0, int g0, int b0, int a0) {
        float dv = (float) (nx * viewDir.field_1352 + ny * viewDir.field_1351 + nz * viewDir.field_1350);
        if (dv < 0.0f) dv = 0.0f;
        if (dv > 1.0f) dv = 1.0f;

        float dl = (float) (nx * lightDir.field_1352 + ny * lightDir.field_1351 + nz * lightDir.field_1350);
        dl = (dl + 1.0f) * 0.5f;
        if (dl < 0.0f) dl = 0.0f;
        if (dl > 1.0f) dl = 1.0f;
