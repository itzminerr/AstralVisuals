...
public final class GlassStyle implements QuickImports {
   ...
   private static void glassPanel(class_4587 matrix, float x, float y, float w, float h, Vector4f round, float tintRole) {
      ...
      boolean canBlur = blurRadius >= 1.0F && blur.input != null;
      if (canBlur) {
         // Настоящее матовое стекло. Во время анимаций прозрачности blur-шейдер рисует фон
         // непрозрачным и не учитывает глобальную альфу (тонировки умножаются на неё, а фон — нет),
         // поэтому плавно уменьшаем силу размытия вместе с альфой: слабое размытие почти
         // неотличимо от живого фона, и стекло «проявляется» без резкого включения.
