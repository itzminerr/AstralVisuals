...
public class AlphaComponent extends AbstractComponent {
   ...
   public void render(class_332 context, int mouseX, int mouseY, float delta) {
      ...
      this.X = this.x + 8.0F;
      this.Y = this.y + 108.0F;
      this.W = this.width - 16.0F;
      this.H = 6.0F;
      float handleSize = 8.0F;
      float clampedX = class_3532.method_15363(this.X + this.W * this.setting.getAlpha() - handleSize / 2.0F, this.X, this.X + this.W - handleSize);
      image.setTexture("textures/gui/alphabar.png").render(ShapeProperties.create(matrix, this.X, this.Y, this.W, this.H).round(4.0F).build());
      rectangle.render(
         ShapeProperties.create(matrix, this.X, this.Y, this.W, this.H)
            .round(this.H / 2.0F)
   ...
   public boolean mouseClicked(double mouseX, double mouseY, int button) {
      this.alphaDragging = button == 0 && Calculate.isHovered(mouseX, mouseY, this.X, this.Y, this.W, this.H);
      if (this.alphaDragging) {
         this.applyValue(mouseX);
         return true;
      } else {
   ...
   public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
      if (this.alphaDragging && button == 0) {
         this.applyValue(mouseX);
         return true;
      } else {
   ...
   private void applyValue(double mouseX) {
      this.setting.setAlpha(class_3532.method_15363((float)(mouseX - this.X) / this.W, 0.0F, 1.0F));
   }
